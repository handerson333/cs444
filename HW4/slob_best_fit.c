 ...
 * 
 * Edited by Thomas Noelcke, Hayden Anderson, James Zeng
 */
...
{
	slob_t *prev, *cur, *best_prev, *best, *best_next,  *aligned = NULL;
	...
	for (prev = NULL, cur = sp->freelist; ; prev = cur, cur = slob_next(cur)) {
		...
		if (avail >= data_sz && (best == NULL || ((avail - data_sz) < best_sz_delta))) { /* room enough? */
			best_prev = prev;
			best = cur;
			best_sz_delta = avail - data_sz;

			//if we found the exact size then break.
			if(best_sz_delta == 0){
				break;
			}
		}
	...
	//moved this logic out of the loop.
	if(best != NULL){
		...
		if (avail == units) { /* exact fit? unlink. */
			...
			set_slob(best + units, avail - units, best_ next);
		}
...
// we will use this later as a system call to calculate
...
// We will use as a system call later to get the amount of
