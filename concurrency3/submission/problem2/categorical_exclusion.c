...
// Prof: Kevin McGrath
// Concurrency assignment 3 problem 2
// sources:
...
void* delete(void* void_head){
	...
	while(1){
		int value = rng(1,10);
		int result = 0;
		// lock mutex for all types
		pthread_mutex_lock(&search_mutex);
		pthread_mutex_lock(&insert_mutex);
		pthread_mutex_lock(&delete_mutex);
		// point pointers around head do delete the node
...
void* insert(void* void_head){
	...
	while(1){
		...
		//move to end of list and insert
		node *current = head;
...
void* search(void* void_head){
	...
	while(1){
		...
		int result = 0;
		pthread_mutex_lock(&search_mutex);
		// Search through entire list until end. 
		...
		pthread_mutex_unlock(&search_mutex);
		// print whether value was found or not
		if(result == 1){
